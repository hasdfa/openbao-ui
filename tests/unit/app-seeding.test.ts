...
import { seedAppConfigs } from "@/lib/apps";
...
describe("app config seeding", () => {
  ...
  it("rejects KV v1 before any environment is written", async () => {
    await expect(seedAppConfigs("app", [{ mount: "dev", v2: true }, { mount: "legacy", v2: false }], "")).rejects.toThrow(/KV v1/);
  ...
  it("reports a partial failure without continuing to later environments", async () => {
    ...
    await expect(seedAppConfigs("app", [{ mount: "dev", v2: true }, { mount: "prod", v2: true }, { mount: "later", v2: true }], "")).rejects.toThrow(/earlier environments/);
